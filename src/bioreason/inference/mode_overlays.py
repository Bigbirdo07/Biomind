...
MODE_OVERLAYS = {
    ...
    "PIPELINE_BUILD": (
        ...
        "WHEN CHAINING SHELL COMMANDS (e.g. bwa/samtools/gatk/bcftools "
        "pipelines), NEVER put a literal `'|'` inside a `subprocess.run(...)` "
        "or `subprocess.Popen(...)` argument LIST — subprocess never "
        "interprets shell metacharacters that way; the `|` is passed as a "
        "literal, meaningless argument to the command. Either pass "
        "`shell=True` with a single command STRING (e.g. "
        "`subprocess.run('bwa mem ref.fa in.fq | samtools sort -o out.bam', "
        "shell=True)`), or chain separate `Popen` calls manually via "
        "`stdout=subprocess.PIPE`.\n"
        "FOR VARIANT CALLING, USE THE CORRECT CALLER FOR THE DESIGN. "
        "`HaplotypeCaller` is GATK's GERMLINE caller — it assumes a normal "
        "diploid genotype model and is the wrong tool for tumor samples. "
        "For somatic variant calling (tumor-only OR tumor-normal paired), "
        "use `Mutect2` instead, and filter its output with "
        "`FilterMutectCalls` (which specifically expects Mutect2's output "
        "fields — do not apply it to HaplotypeCaller output, and do not "
        "invent generic `--filter-expression`/`--filter-name` arguments for "
        "it that aren't part of its real interface). Use the modern GATK4 "
        "command-line wrapper (`gatk ToolName ...`), not the older "
        "`java -jar gatk.jar ToolName` invocation style.\n"
        "WHEN THE USER HAS A MATCHED TUMOR-NORMAL DESIGN, THE CODE MUST "
        "ACTUALLY USE THE PAIRING, NOT JUST PASS BOTH BAMS. Passing both "
        "tumor and normal BAMs to Mutect2 via two `--input`/`-I` flags is "
        "NOT enough — without a `-normal <sample_name>` (or "
        "`--normal-sample`) flag naming which input is the matched normal "
        "(matched by the `SM:` read-group sample name inside that BAM, not "
        "the file path), Mutect2 treats every input as a tumor sample and "
        "silently discards the entire benefit of having a paired normal. "
        "This is the same class of mistake as claiming an aggregation "
        "happened without actually performing the reduction — the code "
        "must implement the design that was actually agreed on, not just "
        "reference the right files.\n"
        "WHEN CHAINING SHELL COMMANDS, `shell=True` REQUIRES A SINGLE "
        "COMMAND STRING, NEVER A LIST. `subprocess.run(['bwa', 'mem', ref, "
        "r1, r2, '|', 'samtools', 'sort', ...], shell=True)` is still "
        "broken even with shell=True added — with shell=True, only the "
        "first list item actually runs as the shell command, and every "
        "other item becomes a positional shell parameter, never appended "
        "to the command line. Build one command string instead: "
        "`subprocess.run(f'bwa mem {ref} {r1} {r2} | samtools sort -o "
        "{out_bam}', shell=True)`."
    ),
    "PIPELINE_DEBUG": (
