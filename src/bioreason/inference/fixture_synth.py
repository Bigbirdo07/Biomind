...
def _build_shell_dry_run_preamble() -> str:
    ...
    return (
        ...
        "        return 0\n"
        "\n"
        "def _fx_check_shell_usage(_fx_cmd, _fx_kwargs):\n"
        "    # Faithfully replicate ONE real subprocess constraint instead of\n"
        "    # silently accepting anything: without shell=True, a plain STRING\n"
        "    # command is run as a single literal executable name with no\n"
        "    # argument splitting -- a multi-word string like\n"
        "    # 'gatk BaseRecalibrator -R ref.fa ...' is not shell-parsed and\n"
        "    # raises FileNotFoundError, since no program is literally named\n"
        "    # that whole string. Seen live: missing shell=True on a plain\n"
        "    # (non-piped) multi-word gatk command string.\n"
        "    _fx_shell = _fx_kwargs.get('shell', False)\n"
        "    if not _fx_shell and isinstance(_fx_cmd, str) and len(_fx_cmd.split()) > 1:\n"
        "        raise FileNotFoundError(2, 'No such file or directory', _fx_cmd)\n"
        "\n"
        "def _fx_mock_run(*_fx_args, **_fx_kwargs):\n"
        "    _fx_cmd = _fx_args[0] if _fx_args else _fx_kwargs.get('args')\n"
        "    _fx_check_shell_usage(_fx_cmd, _fx_kwargs)\n"
