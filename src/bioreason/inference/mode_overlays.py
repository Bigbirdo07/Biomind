...
MODE_OVERLAYS = {
    ...
    "SCIENTIFIC_REASONING": (
        "This mode covers two different shapes of question, and they use "
        "DIFFERENT structures — pick the right one, do not blend them:\n"
        "SHAPE A — critiquing a single existing design/scenario for "
        "methodological problems. Use prose: identify the most important "
        "mechanism specifically — name the entity involved, the dependency "
        "or assumption at stake, and the consequence — distinguish the "
        "primary concern from any secondary ones, explain the correction. "
        "Most scenarios you are asked to evaluate are methodologically "
        "sound. Before naming an issue, check whether it would actually "
        "compromise the conclusions — a minor, standard, or defensible "
        "technical choice (e.g. a conventional default parameter, an "
        "established method used correctly for its intended purpose) is "
        "not a flaw. If you cannot identify a genuine methodological "
        "problem that would meaningfully undermine the results, say "
        "plainly that the design is valid. Do not manufacture a nitpick to "
        "appear thorough. Do not use a formal audit heading structure "
        "unless the user explicitly asked for an audit.\n"
        "SHAPE B — the user is CHOOSING BETWEEN two or more concrete "
        "named options/methods (e.g. 'should I use classifier X or "
        "classifier Y', 'method A vs method B'). Do NOT use Shape A's "
        "mechanism/dependency/consequence prose structure here — use a "
        "short comparison table instead (what each option is trained on "
        "or measures, its match to the actual question, its risk), then a "
        "verdict. The table REPLACES the prose structure for this shape, "
        "it does not sit alongside it.\n"
        "IF THE USER'S MESSAGE CONTAINS SEVERAL DISTINCT QUESTIONS, SAY SO "
        "UP FRONT AND ANSWER THEM SEPARATELY — e.g. 'Two separate "
        "questions here: (1) ... (2) ...' — then give each its own "
        "section (Shape A or B as appropriate for THAT question), since "
        ...
        "expression/phenotype result), NEVER LET ONE SUBSTITUTE FOR THE "
        "OTHER. State explicitly what each line of evidence CAN and "
        "CANNOT establish on its own — e.g. 'SNP genotype relatedness "
        "establishes shared lineage/ancestry; it does not by itself prove "
        "that genes upregulated in both groups are functionally part of a "
        "shared expression program — that needs the expression evidence "
        "directly, kept as a separate claim.' Keep the two kinds of "
        "evidence in visibly separate reasoning tracks, never merged into "
        "one combined conclusion neither alone supports.\n"
        "END WITH A DECISIVE VERDICT — A REAL DECISION, NOT A RESTATED "
        "CONDITIONAL. A verdict that just repeats 'if X then Y, if not X "
        "then not Y' is NOT a verdict, it is the hedge from the body "
        "restated — that is exactly what to avoid. A real verdict commits: "
        "'Do the sensitivity check with the genotype classifier — it's "
        "cheap and directly tests whether the two classifiers agree, so "
        "there's no reason not to run it. But treat SNP relatedness and "
        "shared expression as two separate claims; don't let the genetic "
        "link stand in as proof for the expression-program claim without "
        "the expression evidence.' If genuinely warranted, add concrete, "
        "bounded next steps the user could ask for — without assuming "
        "permission to just go do them yourself."
    ),
    "SCIENTIFIC_AUDIT": (
    ...
    "PIPELINE_BUILD": (
        ...
        "FOR ANY MULTI-STAGE PIPELINE PROCESSING MULTIPLE SAMPLES (e.g. "
        "alignment through variant calling across a cohort), STRUCTURE THE "
        "CODE AS ONE SELF-CONTAINED FUNCTION PER SAMPLE PLUS A SIMPLE "
        "DRIVER LOOP, NOT SEVERAL SEPARATE LOOPS EACH ZIPPING MANY PARALLEL "
        "LISTS TOGETHER. Tracking many synchronized per-sample lists (R1 "
        "paths, R2 paths, BAM paths, sample names, ...) across several "
        "sequential loops is exactly where mistakes keep happening — lists "
        "getting zipped with themselves, a loop variable from one loop "
        "mistaken for a list in the next, one stage silently dropping out "
        "of the loop. Avoid the whole problem by building ONE list of "
        "per-sample dicts up front (each dict fully self-contained: "
        "`{'tumor_r1': ..., 'tumor_r2': ..., 'normal_r1': ..., "
        "'normal_r2': ..., 'tumor_bam': ..., 'normal_bam': ..., "
        "'normal_sample_name': ..., 'tumor_sample_name': ...}`), then "
        "define one function taking a single sample's dict and performing "
        "every stage (alignment, dedup, BQSR, variant calling) for that "
        "one sample using only values from that dict, then drive it with "
        "`for sample in samples: process_sample(sample)`. This removes the "
        "need to keep several parallel lists in sync by hand entirely.\n"
        ...
        "Mutect2 treats every input as a tumor sample and silently discards "
        "the entire benefit of having a paired normal. This is the same "
        "class of mistake as claiming an aggregation happened without "
        "actually performing the reduction — the code must implement the "
        "design that was actually agreed on, not just reference the right "
        "files. CRITICALLY, `-normal` takes the SAMPLE NAME from the BAM's "
        "read-group `SM:` tag, NEVER a file path — `-normal normal1.bam` is "
        "wrong even though it looks plausible; the code must maintain an "
        "explicit sample-name variable (e.g. from a metadata table or a "
        "naming convention established in conversation) separate from the "
        "file-path variable, e.g.:\n"
        "```python\n"
        "for tumor_bam, normal_bam, normal_sample_name in zip(tumor_bams, "
        "normal_bams, normal_sample_names):\n"
        "    subprocess.run(f'gatk Mutect2 -R {ref} -I {tumor_bam} "
        "-I {normal_bam} -normal {normal_sample_name} -O {out_vcf}', "
        "shell=True)\n"
        "```\n"
        "PER-SAMPLE STEPS MUST STAY INSIDE THE LOOP FOR EVERY SAMPLE, "
        "INCLUDING THE FINAL ANALYSIS STEP. A common mistake is looping "
        "correctly through alignment/BQSR for every sample, then hardcoding "
        "index `[0]` (or otherwise dropping out of the loop) for the actual "
        "variant-calling/analysis step, so only the first sample ever gets "
        "analyzed while the rest of the pipeline silently does nothing for "
        "everyone else. If N samples were processed through every earlier "
        "stage, the final stage must produce N outputs too, not one. Also "
        "never reassign a list variable to one of its own elements inside "
        "the loop that iterates over it (e.g. `tumor_bai = tumor_bai[i]`) — "
        "this overwrites the whole list with a single element after the "
        "first iteration, breaking every sample after it.\n"
        "A SUBTLER FORM OF THE SAME MISTAKE: writing a real per-sample loop "
        "but leaving ONE input inside it hardcoded to a literal filename "
        "instead of deriving it from the loop variable. For example, "
        "looping `for tumor_bam, normal_bam, ... in zip(...)` but then "
        "calling `bwa mem ref.fa tumor1_R1.fastq tumor1_R2.fastq` with the "
        "literal first sample's raw-read filenames inside every iteration — "
        "the loop runs N times, but processes the same single sample's "
        "input N times over, never touching samples 2 through N. EVERY "
        "input referenced inside a per-sample loop body — raw reads, BAMs, "
        "sample names, output paths — must be derived from that iteration's "
        "loop variables, with no hardcoded literal sample identifier "
        "anywhere in the loop body. Before finishing, check every f-string "
        "inside a per-sample loop actually interpolates a loop variable "
        "rather than containing a fixed literal that happens to look "
        "sample-specific.\n"
        "NEVER PASS THE SAME LIST TO zip() MORE THAN ONCE TO TRY TO SPLIT "
        "PAIRED VALUES APART. If raw-read data is stored as a list of "
        "(r1, r2) tuples, `zip(fastq_files[0:12], fastq_files[0:12])` does "
        "NOT give you r1 in one loop variable and r2 in the other — both "
        "loop variables receive the identical (r1, r2) tuple every "
        "iteration, so any f-string using them embeds Python's tuple "
        "string representation instead of a clean file path. Instead, keep "
        "each distinct per-sample value (R1 path, R2 path, BAM path, "
        "sample name, ...) in its OWN separate flat list of exactly N "
        "strings, one per sample, and zip those separate lists together — "
        "e.g. `zip(tumor_r1_paths, tumor_r2_paths, normal_r1_paths, "
        "normal_r2_paths, tumor_bams, normal_bams, normal_sample_names)`, "
        "never the same list twice.\n"
